...
import { hospitalKeywords as searchKeywords } from '@/data/hospitalKeywords';
// Props
type Props = Readonly<{
  keyword: string;
  departments: string[];
...
// Page
export default function Map({ keyword, departments }: Props) {
  // Hooks
        ...
        (data: SearchResult[], status: kakao.maps.services.Status) => {
          ...
          if (status === kakao.maps.services.Status.OK) {
            ...
            const results = data.filter((d) =>
              d.category_name.startsWith(`의료,건강 > 병원 > ${department}`)
            );
            ...
            setSearchResults((prev) => {
              const newResults = [
                ...prev,
                ...results.filter((a) => !prev.some((b) => b.id === a.id)),
              ];
  ...
  }, [center, currentLocation, departments, keyword, map]);
  // Pin markers on map
  useEffect(() => {
    if (!map) return;
